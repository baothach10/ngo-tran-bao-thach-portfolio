...
import { AnimationAction, Color, Raycaster, Vector2, Vector3 } from 'three';
...
const ThreeScene = () => {
  ...
  useEffect(() => {
    ...
    models['manInVest'].scene.position.set(0, 0, 4);
  ...
  useEffect(() => {
    if (!camera || !webglRenderer || !allowInteraction) return;
  ...
  function playRandomAnimation(animationActions: { [key: string]: AnimationAction }) {
    ...
    while (
      nonIdleAnimations[randomIndex] === currentAnimationActionsRef.current &&
      nonIdleAnimations.length > 1
    ) {
    ...
    gsap.to(
      {},
      {
        ...
        onStart: () => {
          ...
          if (randomAnimation.getClip().name === 'Bow') disableModelLookAtMouse();
  ...
  return (
    <div className="three-scene">
      <div className="three-scene-container" ref={mountRef}></div>
    </div>
  );
};
